...
public sealed class RecognizeAccrualHandler
{
    private readonly IAccrualRepository _repository;
    private readonly IClock _clock;

    public RecognizeAccrualHandler(IAccrualRepository repository, IClock clock)
    {
    ...
    public async Task<ApplicationResult<AccrualDto>> HandleAsync(
        RecognizeAccrualCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            load.Value!.Recognize(_clock.UtcNow);
        }
