...
public sealed class ChangeAccrualDescriptionHandler
{
    private readonly IAccrualRepository _repository;
    private readonly IClock _clock;

    public ChangeAccrualDescriptionHandler(IAccrualRepository repository, IClock clock)
    {
    ...
    public async Task<ApplicationResult<AccrualDto>> HandleAsync(
        ChangeAccrualDescriptionCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            load.Value!.ChangeDescription(command.Description, _clock.UtcNow);
        }
