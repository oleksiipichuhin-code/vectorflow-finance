...
public sealed class SuspendFinanceWorkspaceHandler
{
    private readonly IFinanceWorkspaceRepository _repository;
    private readonly IClock _clock;

    public SuspendFinanceWorkspaceHandler(IFinanceWorkspaceRepository repository, IClock clock)
    {
    ...
    public async Task<ApplicationResult<FinanceWorkspaceDto>> HandleAsync(
        SuspendFinanceWorkspaceCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            load.Value!.Suspend(_clock.UtcNow);
        }
