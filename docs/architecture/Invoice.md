...
- list invoices for a finance workspace with paging (`page`, `pageSize`, optional exact `status` of `Draft` or `Issued`, optional inclusive `createdFromUtc` / `createdToUtc`, optional exact `documentNumber`, optional exact `counterpartyReference`; CreatedAt descending, then InvoiceId descending; empty page when none; returns `items`, `page`, `pageSize`, `totalCount`);
...
`IInvoiceRepository` is the Application persistence port (`GetByIdAsync`, `ListByWorkspaceAsync`, `ListByDocumentNumberAsync`, and `ListPagedAsync` always workspace-scoped, `AddAsync`, `SaveChangesAsync`). Optional paged `status` filter is exact enum match only. Optional paged CreatedAt bounds are inclusive absolute `DateTimeOffset` instants (`createdFromUtc` / `createdToUtc`; either alone allowed; `from > to` is validation failure). Optional paged `documentNumber` is a positive exact ordinal match after the same trim/normalization as list-by-document-number (missing means no DocumentNumber filter; blank/whitespace and overlength are ValidationFailed; no partial/prefix/full-text mode). Optional paged `counterpartyReference` is a positive exact Ordinal match after `CounterpartyReference` trim/normalization (missing means no CounterpartyReference filter; blank/whitespace and overlength are ValidationFailed; no partial/prefix/case-insensitive/full-text mode). Remaining multi-field filters and full-text search remain later slices. Issue does not create journal entries or ledger postings.

## Persistence (F4E3)

Invoice aggregates are stored via EF Core in Infrastructure:

- `InvoiceRepository` implements `IInvoiceRepository` with workspace-scoped `GetByIdAsync`, `ListByWorkspaceAsync`, `ListByDocumentNumberAsync`, and `ListPagedAsync` (CreatedAt descending, then Id descending). Paged path: SQL filters FinanceWorkspaceId, optional Status, optional exact DocumentNumber, and optional exact CounterpartyReference, then a single materialization with lines; optional inclusive CreatedAt bounds, `totalCount`, Order, Skip, and Take run in memory because the SQLite provider cannot translate DateTimeOffset comparisons or ORDER BY DateTimeOffset (conscious MVP trade-off; storage normalization / indexing may be a later optimization slice);
...
## HTTP surface (F4E4 / F4J / F4M / F4N / F4O / F4P / F4U / F4X)
...
| GET | `/?page={page}&pageSize={pageSize}&status={status?}&createdFromUtc={from?}&createdToUtc={to?}&documentNumber={number?}&counterpartyReference={reference?}` | List invoices for workspace (paged, newest first; optional status, CreatedAt range, document number, and counterparty reference) | 200 |
...
Status mapping via existing `ApplicationResultHttp`: ValidationFailed → 400, NotFound → 404, Conflict → 409. Single-invoice responses use Application `InvoiceDto`. List-by-document-number returns a JSON array of `InvoiceDto` (empty array when none; not 404). Paged list returns Application `PageResult<InvoiceDto>` with `items`, `page`, `pageSize`, and `totalCount` (empty `items` when none; not 404). List and list-by-document-number are read-only. Ordering: `CreatedAt` descending, then `InvoiceId` descending. Paged list requires `page >= 1` and `1 <= pageSize <= 100`; invalid paging is ValidationFailed. Optional paged query parameter `status` accepts exact Ordinal `Draft` or `Issued` only; missing `status` means no status filter (F4N all-status behavior); explicitly blank, whitespace, case variants, numeric values, or unknown names are ValidationFailed. Optional paged query parameters `createdFromUtc` and `createdToUtc` are nullable `DateTimeOffset` absolute instants; either bound alone is allowed; when both are present they form an inclusive closed interval and `createdFromUtc > createdToUtc` is ValidationFailed; equal bounds match the exact instant; offsets are compared as absolute moments (not forced to zero offset); malformed date values fail ASP.NET binding with HTTP 400. Optional paged query parameter `documentNumber` uses the same trim/normalization and blank/overlength ValidationFailed posture as list-by-document-number; missing means no DocumentNumber filter; when provided, only exact ordinal matches are returned (positive filter only; no partial/prefix/suffix/case-insensitive/full-text mode). Optional paged query parameter `counterpartyReference` uses existing `CounterpartyReference` trim/normalization and blank/overlength ValidationFailed posture; missing means no CounterpartyReference filter; when provided, only exact Ordinal matches on `Invoice.CounterpartyReference` are returned (positive filter only; no partial/prefix/suffix/case-insensitive/full-text mode). Workspace, optional status, optional document-number, and optional counterparty-reference filtering remain SQL predicates; CreatedAt range filtering, `totalCount`, ordering, and Skip/Take run in memory after one materialization because SQLite cannot translate DateTimeOffset comparisons or ORDER BY DateTimeOffset (no schema/index/migration change in F4X). List-by-document-number filters by workspace and exact ordinal `DocumentNumber` after trim; blank/overlength document numbers are ValidationFailed; multiple invoices may share one document number; there is no DocumentNumber index or uniqueness guarantee. Paged list does not include remaining multi-field filters, full-text search, or client-controlled sort. List-by-document-number does not include pagination or total-count metadata.
