...
- list invoices for a finance workspace with paging (`page`, `pageSize`, optional exact `status` of `Draft` or `Issued`, optional inclusive `createdFromUtc` / `createdToUtc`, optional exact `documentNumber`; CreatedAt descending, then InvoiceId descending; empty page when none; returns `items`, `page`, `pageSize`, `totalCount`);
...
`IInvoiceRepository` is the Application persistence port (`GetByIdAsync`, `ListByWorkspaceAsync`, `ListByDocumentNumberAsync`, and `ListPagedAsync` always workspace-scoped, `AddAsync`, `SaveChangesAsync`). Optional paged `status` filter is exact enum match only. Optional paged CreatedAt bounds are inclusive absolute `DateTimeOffset` instants (`createdFromUtc` / `createdToUtc`; either alone allowed; `from > to` is validation failure). Optional paged `documentNumber` is a positive exact ordinal match after the same trim/normalization as list-by-document-number (missing means no DocumentNumber filter; blank/whitespace and overlength are ValidationFailed; no partial/prefix/full-text mode). Remaining multi-field filters and full-text search remain later slices. Issue does not create journal entries or ledger postings.

## Persistence (F4E3)

Invoice aggregates are stored via EF Core in Infrastructure:

- `InvoiceRepository` implements `IInvoiceRepository` with workspace-scoped `GetByIdAsync`, `ListByWorkspaceAsync`, `ListByDocumentNumberAsync`, and `ListPagedAsync` (CreatedAt descending, then Id descending). Paged path: SQL filters FinanceWorkspaceId, optional Status, and optional exact DocumentNumber, then a single materialization with lines; optional inclusive CreatedAt bounds, `totalCount`, Order, Skip, and Take run in memory because the SQLite provider cannot translate DateTimeOffset comparisons or ORDER BY DateTimeOffset (conscious MVP trade-off; storage normalization / indexing may be a later optimization slice);
...
## HTTP surface (F4E4 / F4J / F4M / F4N / F4O / F4P / F4U)
...
| GET | `/?page={page}&pageSize={pageSize}&status={status?}&createdFromUtc={from?}&createdToUtc={to?}&documentNumber={number?}` | List invoices for workspace (paged, newest first; optional status, CreatedAt range, and document number) | 200 |
