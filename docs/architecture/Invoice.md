...
- list invoices for a finance workspace with paging (`page`, `pageSize`, optional exact `status` of `Draft` or `Issued`, optional inclusive `createdFromUtc` / `createdToUtc`, optional exact `documentNumber`, optional exact `counterpartyReference`, optional exact `currency`, optional inclusive `issuedFromUtc` / `issuedToUtc`, optional inclusive `dueFromUtc` / `dueToUtc`, optional inclusive `totalAmountFrom` / `totalAmountTo`; CreatedAt descending, then InvoiceId descending; empty page when none; returns `items`, `page`, `pageSize`, `totalCount`);
...
- `UpdatedAt` is an EF Core optimistic concurrency token (no extra column/migration): concurrent saves after a stale load fail with `DbUpdateConcurrencyException`, mapped by `InvoiceRepository.SaveChangesAsync` to `InvalidOperationException` → Application Conflict (HTTP 409);
...
## HTTP surface (F4E4 / F4J / F4M / F4N / F4O / F4P / F4U / F4X / F4Y / F4AB / F4AC / F4AD / F4AK)
...
| GET | `/?page={page}&pageSize={pageSize}&status={status?}&createdFromUtc={from?}&createdToUtc={to?}&documentNumber={number?}&counterpartyReference={reference?}&currency={currency?}&issuedFromUtc={issuedFrom?}&issuedToUtc={issuedTo?}&dueFromUtc={dueFrom?}&dueToUtc={dueTo?}&totalAmountFrom={amountFrom?}&totalAmountTo={amountTo?}` | List invoices for workspace (paged, newest first; optional status, CreatedAt range, document number, counterparty reference, currency, IssuedAt range, DueDate range, and TotalAmount range) | 200 |
