...
F4A–F4AG are published. F4AG Accrual Paged ReversedAt Range Composition (optional inclusive `reversedFromUtc` / `reversedToUtc` on paged Accrual listing filtering lifecycle `ReversedAt`; either bound alone allowed; equal bounds match that exact instant; `reversedFromUtc > reversedToUtc` is ValidationFailed; when any ReversedAt bound is present, null `ReversedAt` rows are excluded; independent of optional `status` and of RecognizedAt/RecognitionDate bounds; composes under AND with optional exact `status`, inclusive CreatedAt/RecognitionDate/Amount/RecognizedAt bounds, exact `sourceInvoiceId`, exact `type`, exact `currency`, and exact `description`; ReversedAt bounds applied in memory with existing DateTimeOffset filter stage; no Domain, migration, schema, package, Contracts, or DI changes; `ReversalReason` filtering remains deferred) is published. Remaining invoice query enhancements (other multi-field filters / full-text search), further Accrual text modes, and other deferred Accrual query capabilities remain planned under F4Q+. F4 as a whole remains incomplete.
