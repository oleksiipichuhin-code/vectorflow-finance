...
F4A–F4AH are published. F4AH Accrual Paged ReversalReason Composition (optional exact Ordinal `reversalReason` on paged Accrual listing; missing means no ReversalReason filter; when provided, trim + blank/overlength ValidationFailed using Domain ReversalReason max 500; positive exact match only — null ReversalReason rows do not match; no partial/prefix/case-insensitive/full-text; composes under AND with optional exact `status`, inclusive CreatedAt/RecognitionDate/Amount/RecognizedAt/ReversedAt bounds, exact `sourceInvoiceId`, exact `type`, exact `currency`, and exact `description`; ReversalReason filter applied as SQL predicate with other exact string filters; no Domain, migration, schema, package, Contracts, or DI changes) is published. Remaining invoice query enhancements (other multi-field filters / full-text search), further Accrual text modes (prefix/contains/full-text), and other deferred Accrual query capabilities remain planned under F4Q+. F4 as a whole remains incomplete.
